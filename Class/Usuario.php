...
class Usuario {
	...
	public function login($login, $password){
		...
		$results = $sql->select("SELECT * FROM tb_usuarios WHERE desloguin = :LOGIN AND dessenha = :PASSWORD", array(
			":LOGIN"=>$login,
			":PASSWORD"=>$password
	    ));
		...
		if (count($results) > 0){
			...
			throw new Exception("Login e/ou senha invalods.");
	...
	public function update($Login, $password){
		...
		$sql->query("UPDATE tb_usuarios SET desloguin = :LOGIN, dessenha = :PASSWORD WHERE idusuarios = :ID", array(
